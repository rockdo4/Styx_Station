...
public class TutorialSystem : MonoBehaviour
{
    ...
    {
        if (tutorials[tutorialIndex].index.Count == playIndex)
        {
    ...
    public void Log()
    {
        if(tutorials[tutorialIndex].index[playIndex].stringKey.Contains("Tutorial_Log"))
        {
            message = true;
            ...
            if (Global.language == Language.KOR)
                str = stringTable.GetStringTableData(tutorials[tutorialIndex].index[playIndex].stringKey).KOR;
            else if (Global.language == Language.ENG)
                str = stringTable.GetStringTableData(tutorials[tutorialIndex].index[playIndex].stringKey).ENG;
    ...
    public void Finger()
    {
        if (tutorials[tutorialIndex].index[playIndex].fingerObj != null &&
            !tutorials[tutorialIndex].index[playIndex].stringKey.Contains("Tutorial_Log"))
        {
            tutorials[tutorialIndex].index[playIndex].fingerObj.SetActive(true);
        }
    }

    public void Mask()
    {
        if (tutorials[tutorialIndex].index[playIndex].maskObj != null)
        {
            tutorials[tutorialIndex].index[playIndex].maskObj.SetActive(true);
        }
    }
    ...
    private void Update()
    {
        if (Input.anyKeyDown && playTutorial)
        {
            if (!message && next)
            {
                ...
                if (tutorials[tutorialIndex].index[playIndex].maskObj.activeSelf)
                    tutorials[tutorialIndex].index[playIndex].maskObj.SetActive(false);

                if (tutorials[tutorialIndex].index[playIndex].fingerObj.activeSelf)
                    tutorials[tutorialIndex].index[playIndex].fingerObj.SetActive(false);

                playIndex++;

                if ((tutorialIndex == 0 && playIndex == 2) || (tutorialIndex == 0 && playIndex == 18))
                {
                    WaveManager.Instance.StartWave();
                ...
                else if(tutorialIndex == 2 && playIndex == 7)
                {
                    DiningRoomSystem.Instance.counting++;
                }
                else if(tutorialIndex == 3 && playIndex == 0)
                {
                    if(CurrencyManager.money2 <= 500)
